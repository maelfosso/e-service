...
import org.cpc.yaounde.eservice.utils.CircleTransform;
 ...
 */
public class MessagesListAdapter extends RecyclerView.Adapter<MessagesListAdapter.ViewHolder> implements Filterable {
    public static String TAG = MessagesListAdapter.class.getName();
    ...
    public MessagesListAdapter(Context ctx, List<Message> items) {
        this.ctx = ctx;
    ...
    public void onBindViewHolder(ViewHolder holder, final int position) {
        ...
        Log.d(TAG, "Size " + filtered_items.size());
        Log.d(TAG, c.getUser().toString());
        holder.title.setText(c.getUser().getName());

        holder.time.setText(c.getDate());
        holder.content.setText(c.getSnippet());
        Picasso.with(ctx).load(ctx.getResources().getIdentifier(c.getUser().getPhoto(), null, ctx.getPackageName()))
                .resize(100, 100)
    ...
    public void removeSelectedItem(){
        List<Message> items = getSelectedItems();
    ...
    private class ItemFilter extends Filter {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            for (int i = 0; i < list.size(); i++) {
                String str_title = list.get(i).getUser().getName();
