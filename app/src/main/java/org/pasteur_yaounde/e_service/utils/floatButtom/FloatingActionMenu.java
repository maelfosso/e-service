package org.pasteur_yaounde.e_service.utils.floatButtom;
...
import org.pasteur_yaounde.e_service.utils.anim_action_user.DefaultAnimationHandler;
import org.pasteur_yaounde.e_service.utils.anim_action_user.MenuAnimationHandler;
...
public class FloatingActionMenu {
     ...
     * @param animated
     */
    public FloatingActionMenu(final View mainActionView, int startAngle, int endAngle, int radius, List<Item> subActionItems,
                              MenuAnimationHandler animationHandler, boolean animated, MenuStateChangeListener stateChangeListener,
                              final boolean systemOverlay) {
        Log.i("Jointure :", "3 _ Constructeur de FloatinActionMenu...");
        ...
        if(animationHandler != null)    animationHandler.setMenu(this);

        if(systemOverlay)    overlayContainer = new FrameLayout(mainActionView.getContext());
        else    overlayContainer = null; // beware NullPointerExceptions!
        ...
        Log.i("Jointure :", "4 _ Attribution d'un contenu à chacun des éléments...");
        ...
        Log.i("Jointure :", "5 _ Fin du constructeur de la classe FloatingActionMenu ...");
    }
    ...
    public void open(boolean animated) {
        ...
        if(animated && animationHandler != null) {
            ...
            for (int i = 0; i < subActionItems.size(); i++) {
                ...
                if(systemOverlay)    params.setMargins(center.x - overlayParams.x - subActionItems.get(i).width / 2,
                                                       center.y - overlayParams.y - subActionItems.get(i).height / 2, 0, 0);
                else    params.setMargins(center.x - subActionItems.get(i).width / 2,
        ...
        if(stateChangeListener != null)    stateChangeListener.onMenuOpened(this);
    }
    ...
    public void close(boolean animated) {
        ...
        if(animated && animationHandler != null) {
            ...
            for (int i = 0; i < subActionItems.size(); i++)    removeViewFromCurrentContainer(subActionItems.get(i).view);
            detachOverlayContainer();
        }
        // do not forget to specify that the menu is now closed.
        open = false;
        if(stateChangeListener != null)    stateChangeListener.onMenuClosed(this);
    }
    ...
    private Point calculateItemPositions() {
        ...
        int divisor;
        if(Math.abs(endAngle - startAngle) >= 360 || subActionItems.size() <= 1)    divisor = subActionItems.size();
        else    divisor = subActionItems.size() -1;
    ...
    private void addViewToCurrentContainer(View view, ViewGroup.LayoutParams layoutParams) {
        if(systemOverlay)    overlayContainer.addView(view, layoutParams);
        else {
            try {
    ...
    private WindowManager.LayoutParams calculateOverlayContainerParams() {
        ...
        for(int i=0; i < subActionItems.size(); i++) {
            ...
            if(lm < left)    left = lm;
            if(tm < top)    top = tm;
            if(lm + subActionItems.get(i).width > right)    right = lm + subActionItems.get(i).width;
            if(tm + subActionItems.get(i).height > bottom)    bottom = tm + subActionItems.get(i).height;
        }
    ...
    public int getStatusBarHeight() {
        ...
        if (resourceId > 0)    result = mainActionView.getContext().getResources().getDimensionPixelSize(resourceId);
        return result;
    }
    ...
    public void removeViewFromCurrentContainer(View view) {
        if(systemOverlay)    overlayContainer.removeView(view);
        else    ((ViewGroup)getActivityContentView()).removeView(view);
    }
    ...
    public static class Builder {
        ...
        /**
         * Methode permettant de nettoyer le contenu de notre objet
         */
        public void removeSubActionView(){
            if(subActionItems.size() > 0)     subActionItems.removeAll(subActionItems);
            Log.i("Vidange 1 :", "Vidange du conteneur d'éléments du menu");
        }
        ...
        public Builder attachTo(View actionView) {
            ...
            Log.i("Jointure :", "1 _ joindre les éléments àun bouton pour affichage...");
            return this;
        }

        public FloatingActionMenu build() {
            Log.i("Jointure :", "2 _ joindre les éléments àun bouton pour affichage...");
